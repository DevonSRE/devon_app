...
const ChooseUs = () => {
  return (
    <div className="overflow-hidden">
      <div
        className={cn(
          "py-28 md:py-60 px-5 md:px-0 mx-auto",
          "relative max-w-screen-sm md:max-w-screen-xl",
          "grid grid-cols-1 md:grid-cols-12 gap-4 md:gap-8 gap-x-5 md:gap-x-10",
        )}
      >
        {/* patterns top right */}
        <Image
        ...
        <div className="col-start-1 md:col-start-2 col-end-2 md:col-end-12 grid grid-cols-1 md:grid-cols-12 gap-6 md:gap-12">
          <div className="h-full md:h-48 w-full md:w-72 max-w-full col-span-full md:col-span-4">
            <SectionTitle className="mb-2.5 text-2xl md:text-4xl">
              Why Choose Us
            </SectionTitle>
            <p className="font-light text-sm md:text-base">
              We understand the unique challenges of the legal sector. We are
              commited to empowering legal professionals to do their best work.
            </p>
          </div>
...
const TiltingCard = ({
  ...
  return (
    <motion.div
      ...
      className={cn(
        "relative group",
        "p-5 pt-0 bg-neutral-100",
        "rounded-xl space-y-1 md:space-y-2",
        "duration-500 ease-out transition-colors",
        "h-fit md:h-48 w-full md:w-80 max-w-full hover:bg-devon-green hover:text-white",
        className,
      )}
    >
      <div className="relative bg-white/40 w-10 h-12 p-1 text-devon-green group-hover:text-white group-hover:bg-neutral-100/20 rounded-br-xl">
        <div className="absolute bottom-1">{icon}</div>
      </div>

      <p className="font-semibold text-neutral-950 group-hover:text-white">
        {heading}
      </p>

      <p className="font-light text-xs text-neutral-950 group-hover:text-white">
        {body}
      </p>
    </motion.div>
  );
