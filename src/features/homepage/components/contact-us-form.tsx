...
const ContactusForm = ({ className }: { className?: string }) => {
  ...
  const [errors, setErrors] = useState<TValues>(initialErrors);
  const [touched, setTouched] = useState<Partial<TValues>>({});
  ...
  const handleChange = ({
    ...
    if (touched[target.name as keyof TValues]) {
      setErrors((prev) => ({
        ...prev,
        [target.name]: validate()[target.name as keyof TValues] || "",
      }));
    }
  };
