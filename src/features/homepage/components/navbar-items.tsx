...
export function NavbarItems({ navItems }: { navItems: TNavbarItem[] }) {
    ...
    <nav className="flex items-center justify-between gap-4">
      ...
      <div className="menu-bar lg:hidden">
        <Button
          variant={"ghost"}
          size={"icon"}
          className={cn("h-8 w-8 [&_svg]:size-5", {
            "text-black hover:bg-black/10": navbarOpen,
            "hover:bg-black/15 text-black": !navbarOpen,
          })}
          ...
          <Fragment key={item.id}>
            {item.category === "links" && (
              <li key={item.id} className="relative">
                <Link
                  href={item.url}
                  className={cn("py-0.5 px-1 size-full font-light", {
                    "after:absolute after:w-full after:h-1.5 after:bg-white after:z-10 after:left-0 after:-bottom-6 font-medium":
                      pathname === item.url,
                  })}
                >
                  {item.value}
