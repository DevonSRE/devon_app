...
const contacts: {
...
}[] = [
  {
    id: 1,
    type: "phone",
    value: "+234 810 426 8727",
    icon: <PhoneCallIcon size={24} className="size-4 md:size-6" />,
    subText: "Copy to Clipboard",
  },
  {
    id: 2,
    type: "mail",
    value: "info@devontech.io",
    icon: <MailIcon size={24} className="size-4 md:size-6" />,
    subText: "Send a Mail",
  },
  {
    id: 3,
    type: "map",
    value:
      "Mercy Samuelson Estate, Block L, House No. 2, Plot 622, Karmo district, Abuja, Federal Capital Territory",
    icon: <MapPinnedIcon size={24} className="size-4 md:size-6" />,
    subText: "View of Google Maps",
  },
];
...
const ContactCard = (props: {
  ...
  const handleClick = (type: "phone" | "mail" | "map") => {
    switch (type) {
      case "phone":
  ...
  return (
    <div
      className="hover:cursor-pointer flex flex-col gap-1 md:gap-2"
      title={props.subText}
    >
      <div
      ...
      >
        ...
        <p
          className={cn(
            ...
            props.value.length > 25 ? "w-72 md:w-96" : "whitespace-nowrap",
          )}
        >
          {props.value}
        </p>
      </div>
      <small className="text-[9px] font-light bg-green-100/50 py-1 px-2 w-fit">
        {props.subText}
      </small>
    </div>
  );
