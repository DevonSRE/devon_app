...
const Countdown = ({
  ...
  const targetTime = new Date(
    targetDate.includes("T") ? targetDate : `${targetDate}T23:59:59`,
  ).getTime();
        ...
        {["days", "hours", "minutes", "seconds"].map((label) => (
          ...
          >
            <div className="px-1 md:px-2 py-px md:py-1 w-10 lg:w-16 rounded-md flex flex-col items-center bg-neutral-950 ">
              <motion.span
                key={timeLeft[label as keyof typeof timeLeft]} // Re-animates when value changes
            ...
            <span className="text-[9px] md:text-xs lg:text-sm capitalize text-neutral-400 font-normal">
              {label}
            </span>
          </motion.div>
        ))}
